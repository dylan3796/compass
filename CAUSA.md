...
2. **Own the join and the verdict.** Causa's proprietary layer is the actor model, the run→entity→outcome contribution graph, and the deterministic verdict engine. That is the moat surface; everything else is commodity.
...
5. **ICP wedge:** Series A–C companies spending $20K+/mo on outcome- or usage-priced agents, where a finance or ops leader owns the AI budget — while welcoming the manager-level door (someone's homegrown fleet) as a second entry point. Upmarket and ecosystem motions come after the wedge works.
