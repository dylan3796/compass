...
- `lib/data.ts` numbers are ENGINE OUTPUT: the attribution core lives in its own repo, `dylan3796/causa-engine` (CAUSA.md §6 — join engine, causal ladder A/B/C/D, deterministic verdict engine). This repo consumes one sealed artifact from it, `lib/generated/meridian-ledger.json`, verified by the golden pins in data.ts at build time. Never hand-edit a number in data.ts or the artifact — change the engine's workbook/fixtures, reconcile there, copy the artifact over, and update the pins deliberately.
- Any screen that doesn't end in a decision is a defect.

App: Next.js (App Router) + TypeScript + Tailwind + Framer Motion, at the repo root. `npm run dev` to develop, `npm run build` to verify (the data assertions run at build time). Lead capture posts to `NEXT_PUBLIC_FORMSPREE_ENDPOINT` (see README).
