...
- `lib/data.ts` numbers are ENGINE OUTPUT: the attribution core in `lib/engine` (CAUSA.md §6 — join engine, causal ladder A/B/C/D, deterministic verdict engine) derives the full ledger from event-level fixtures; `npm run reconcile` regenerates `lib/engine/generated/meridian-ledger.json`. Never hand-edit a number in data.ts or the generated JSON — change the workbook/fixtures and re-reconcile. The engine is sealed: no clocks, no `Math.random`, no LLM calls, no React/Next imports, never shipped in the client bundle (all test-enforced; `npm test`).
- Any screen that doesn't end in a decision is a defect.

App: Next.js (App Router) + TypeScript + Tailwind + Framer Motion, at the repo root. `npm run dev` to develop, `npm run build` to verify (reconcile + data assertions run at build time), `npm test` for the engine suite. Lead capture posts to `NEXT_PUBLIC_FORMSPREE_ENDPOINT` (see README).
