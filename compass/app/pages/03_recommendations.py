...
import common
from theme import fmt_money, sev_badge
...
rtype = c3.selectbox("Type", ["All"] + sorted(pending["type"].unique().tolist()))
...
for _, r in view.iterrows():
    ...
    with box:
        ...
        col1.markdown(
            ...
            f'<div class="rec-line" style="margin-top:6px;">{r["description"]}</div></div>',
            unsafe_allow_html=True)
        col2.markdown(
    ...
    with st.expander(f"Resolved ({len(resolved)})"):
        for _, r in resolved.iterrows():
            st.markdown(
                ...
                f'[{r["status"].upper()}] {r["agent_name"]} · {r["type"]} — {r["description"]}</span>',
                unsafe_allow_html=True)
